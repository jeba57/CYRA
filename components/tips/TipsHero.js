...
export default function TipsHero() {
  return (
    <section style={{ minHeight:"50vh", background:"linear-gradient(135deg,#FDF8F3,#F5EEFF,#FDE8E0)", display:"flex", alignItems:"center", padding:"8rem 4rem 4rem", position:"relative", overflow:"hidden" }}>
      <motion.div style={{ position:"absolute", bottom:"-100px", left:"-100px", width:"400px", height:"400px", borderRadius:"50%", background:"radial-gradient(circle,rgba(252,222,192,0.45),transparent 70%)", pointerEvents:"none" }} animate={{ scale:[1,1.06,1] }} transition={{ duration:9, repeat:Infinity }} />
      <div style={{ position:"relative", zIndex:1, maxWidth:"650px" }}>
        ...
        <motion.h1 initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.7, delay:0.1 }}
          ...
          Gentle nudges,<br /><em style={{ fontStyle:"italic", color:"#C8909A" }}>made just for you.</em>
        </motion.h1>
        <motion.p initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.7, delay:0.2 }}
          ...
          Phase-aware, science-backed wellness tips — from Indian home remedies to global self-care rituals, delivered with love.
        </motion.p>
      </div>
    </section>
  );
