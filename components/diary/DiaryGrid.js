...
export default function DiaryGrid() {
  const { files, grouped, loading, deleteFile } = useDiary();

  if (loading) return (
    <section style={{ padding:"2rem 1.5rem 5rem", background:"#FDF8F3" }}>
      <div style={{ maxWidth:"860px", margin:"0 auto", textAlign:"center", color:"#9B7B9E" }}>Loading...</div>
    </section>
  );
    ...
    <section style={{ padding:"2rem 1.5rem 5rem", background:"#FDF8F3" }}>
      <div style={{ maxWidth:"860px", margin:"0 auto" }}>
        ...
        <div style={{ marginBottom:"2.5rem" }}>
          <p style={{ fontSize:"0.75rem", fontWeight:700, letterSpacing:"0.12em", textTransform:"uppercase", color:"#C8909A", marginBottom:"1rem" }}>📁 Categories</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(148px,1fr))", gap:"1rem" }}>
            {Object.entries(CATEGORY_META).map(([key, { icon, label }]) => {
              ...
              return (
                <motion.div key={key}
                  ...
                  <div style={{ fontSize:"0.83rem", fontWeight:700, color:"#3D2840", marginBottom:"0.2rem" }}>{label}</div>
                  <div style={{ fontSize:"0.72rem", color:count>0?"#C8909A":"#9B7B9E", fontWeight:count>0?700:400 }}>
        ...
        {files.length > 0 ? (
          <div>
            <p style={{ fontSize:"0.75rem", fontWeight:700, letterSpacing:"0.12em", textTransform:"uppercase", color:"#C8909A", marginBottom:"1rem" }}>🕐 All Files ({files.length})</p>
              ...
              {files.map((file, i) => {
                ...
                return (
                  <motion.div key={file.id}
                    ...
                    {isImage ? (
                      <div style={{ width:"40px", height:"40px", borderRadius:"10px", overflow:"hidden", flexShrink:0, border:"1px solid rgba(232,160,156,0.2)" }}>
                        <img src={file.fileUrl} alt={file.name} style={{ width:"100%", height:"100%", objectFit:"cover" }} />
                      </div>
                    ) : (
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <p style={{ fontSize:"0.85rem", fontWeight:600, color:"#3D2840", marginBottom:"0.12rem", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                        {file.name}
                      </p>
                      <p style={{ fontSize:"0.7rem", color:"#9B7B9E" }}>
                        {meta.label} · {formatDate(file.uploadedAt)} · {formatSize(file.fileSize)}
                      </p>
                      {file.notes && <p style={{ fontSize:"0.7rem", color:"#9B7B9E", fontStyle:"italic" }}>{file.notes}</p>}
                    </div>

                    <div style={{ display:"flex", gap:"0.5rem", flexShrink:0 }}>
                      {/* View — opens in new tab */}
                      <a href={file.fileUrl} target="_blank" rel="noreferrer" download={file.name}>
                        <button style={{ background:"transparent", border:"1px solid rgba(200,144,154,0.3)", color:"#C8909A", padding:"0.3rem 0.85rem", borderRadius:"50px", fontSize:"0.7rem", fontWeight:600, cursor:"pointer", transition:"all 0.2s" }}
                      ...
                      {/* Delete */}
                      <button onClick={() => { if(confirm(`Delete "${file.name}"?`)) deleteFile(file.id); }}
                        style={{ background:"transparent", border:"1px solid rgba(196,69,105,0.2)", color:"#C44569", padding:"0.3rem 0.65rem", borderRadius:"50px", fontSize:"0.7rem", fontWeight:600, cursor:"pointer", transition:"all 0.2s" }}
          ...
          <div style={{ textAlign:"center", padding:"3rem", background:"#fff", borderRadius:"20px", border:"1px solid rgba(232,160,156,0.15)" }}>
            ...
            <p style={{ fontSize:"0.85rem", color:"#9B7B9E" }}>Upload your first health document above.</p>
          </div>
        )}
      </div>
    </section>
